...
$stmt = $pdo->prepare(" INSERT INTO event (title, description, event_date, location, type, start_time, end_time, capacity, is_alumni_exclusive, created_by, status, approved_by, approved_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())");
$stmt->execute([$event_title, $description, $event_date, $location, $type, $start_time, $end_time, $capacity, $is_alumni_exclusive, $user_id, "approved", $user_id]);
